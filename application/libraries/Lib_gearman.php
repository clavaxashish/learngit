...
class Lib_gearman
{
    ...
    public function gearman_client()
    {
        ...
        $this->_auto_connect_client($this->client);
	 ...
	 * @return void
	 */
	private function _auto_connect_client($object)
	{
		$this->ci->load->config('gearman');
		$this->gearman_host = $this->ci->config->item('gearman_client');
		$this->gearman_port = $this->ci->config->item('gearman_client_port');
		foreach ($this->gearman_host as $key=>$server) {
			if (!$object->addServer($server,$this->gearman_port[$key])) {
				$this->errors[] = "Gearman Library: Could not connect to the client named $key";
				log_message('error', 'Gearman Library: Could not connect to the client named "'.$key.'"');
			} else {
				log_message('debug', 'Gearman Library: Successfully connected to the client named "'.$key.'"');
			}
		}
	}
